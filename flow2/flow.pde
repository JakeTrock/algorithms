...
void draw() {
  ...
  for (int i=0; i<cells.size(); i++) {//implement cell death
    ...
    //mean cells don't play by the rules
    for (Food n : nuggets) {
      if (cells.get(i).getClass()==cell.class) cells.get(i).follow(n.f);
      cells.get(i).colliding(n);//food collision
    }
    if (cells.get(i).getClass()==cell.class) cells.get(i).update();
...
class Water extends Food {//no attraction
